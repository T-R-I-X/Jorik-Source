...
function PromiseUtils.any(promises)
	local returnPromise = PromiseUtils._promise.new()
...
function PromiseUtils.all(promises)
	if #promises == 0 then
		return PromiseUtils._promise.resolved()
	end
...
function PromiseUtils.invert(promise)
	if promise:IsPending() then
		return promise:Then(function(...)
			return PromiseUtils._promise.rejected(...)
		end, function(...)
			return PromiseUtils._promise.resolved(...)
		end)
	else
		...
		if results[1] then
			return PromiseUtils._promise.rejected(unpack(results, 2))
		else
			return PromiseUtils._promise.resolved(unpack(results, 2))
		end
	end
end

function PromiseUtils.fromSignal(signal)
	local promise = PromiseUtils._promise.new()
	local conn
