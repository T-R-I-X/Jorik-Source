...
local raycast = engine.load("Raybox")
...
local function AddDamage(head,damage)
	...
	local tween = tweenService:Create(textLabel,tweenInfo,{ Position=UDim2.new(0,0,-1,0), TextTransparency=1, Roatation=180 })
...
network:GetEvent("Damage").OnServerEvent:Connect(function(client,item,holdingDamage,damage)
	...
	spawn(function()
		...
		hitbox.OnHit:Connect(function(hit)
			...
			if targetHumanoid.Health > 0 and targetPlayer and targetRoot then
